...
一加手机的系统短信和通话记录是唯一数据源。Android 端在本地维护一份 Room 镜像加事务性 outbox，按至少一次的语义发送 HMAC 签名事件，并定期用系统 Provider 的完整快照做对账。服务端是单进程 FastAPI，按 `(device_id, event_id)` 幂等去重，镜像行的唯一身份是 `(device_id, entity_type, source_id)`。
