...
Production configuration refuses weak/default secrets. Never commit `deploy/secrets.env`. Logs may contain event IDs, entity/action and timings, but not bodies, OTPs, phone numbers, cookies, endpoints, keys or passwords. Bark privacy mode is the default and sends only a generic receipt notice.
